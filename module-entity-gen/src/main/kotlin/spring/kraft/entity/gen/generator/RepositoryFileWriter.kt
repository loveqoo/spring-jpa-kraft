...
class RepositoryFileWriter {
    fun write(metadata: EntityMetadata): String {
        ...
        sb.appendLine("import org.springframework.data.querydsl.QuerydslPredicateExecutor")
        sb.appendLine("import spring.kraft.jpa.repo.DynamicSearchRepository")
        sb.appendLine()
        sb.appendLine("interface ${metadata.className}Repository :")
        sb.appendLine("    JpaRepository<${metadata.className}, ${metadata.idType}>,")
        sb.appendLine("    QuerydslPredicateExecutor<${metadata.className}>,")
        sb.appendLine("    DynamicSearchRepository<${metadata.idType}, ${metadata.className}>")
        sb.appendLine()
